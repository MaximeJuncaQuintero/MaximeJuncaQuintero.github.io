...
const projects: Project[] = [
  ...
  },
  {
    ...
    tech:        ['Daily Journaling', 'Knowledge Graph', 'Archiving', 'Idea Linking'],
    tags:        ['Daily Journaling', 'Knowledge Graph', 'Archiving', 'Idea Linking'],
    category:    'consulting',
...
export default function Projects() {
  ...
  const allTags = useMemo(() => {
    ...
    for (const p of projects) {
      const tags = (p.tags?.length ? p.tags : p.tech) ?? []
  ...
  const visible = projects.filter(p => {
    ...
    const tags = (p.tags?.length ? p.tags : p.tech) ?? []
    return activeTags.some(tg => tags.includes(tg))
  })
        ...
        >
          ...
          <div className="mt-5 flex flex-col items-center gap-3">
            ...
            <div className="relative w-full max-w-4xl">
              <div className="pointer-events-none absolute left-0 top-0 h-full w-10 z-10" style={{ background: 'linear-gradient(90deg, var(--bg-alt), rgba(0,0,0,0))' }} />
              <div className="pointer-events-none absolute right-0 top-0 h-full w-10 z-10" style={{ background: 'linear-gradient(270deg, var(--bg-alt), rgba(0,0,0,0))' }} />
